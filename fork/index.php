...
function logData() {
    ...
    $logFile = __DIR__ . "/log.txt";  // 使用完整路径
    $fp = fopen($logFile, "a");
    if ($fp !== false) {
        fwrite($fp, $data."\n");
        fclose($fp);
...
list($username, $branch, $loading_msg, $burl) = explode("/", substr($url, 1));  # todo: clip these strings at the max length in index (to show up on the webpage)
...
$loading_msg = $loading_msg ? substr(trim($loading_msg), 0, 39) : '';
$supplied_loading_msg = $loading_msg != "";  # to print secret message
$burl = $burl ? substr(trim($burl), 0, 255) : '';
...
if ($username == "") {
    ...
    echo "<h2><a href='" . BASE_DIR . "/mouxangithub/dp'><span>" . WEBSITE_URL . BASE_DIR . "/username/branch/msg/gitUrl</span></a></h2>";
...
if(array_key_exists('download_agnos', $_POST)) {
    header("Location: " . BASE_DIR . "/build_agnos.php?username=" . $username . "&branch=" . $branch . "&loading_msg=" . $loading_msg . "&url=" . $burl);
    exit;
}
?>
